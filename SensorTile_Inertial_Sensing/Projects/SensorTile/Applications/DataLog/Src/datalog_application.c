...
// global variables for x displacement
float velocity_x = 0, prev_velocity_x = 0;
float displacement_x = 0, displacement_prev_x = 0;
...
float velocity_filter_x, velocity_filter_prev_x, velocity_prev_x;

float accel_x_filter, accel_x_prev = 0, accel_x_filter_prev = 0;
float displacement_filter_x = 0, displacement_filter_prev_x = 0;

// global variables for y displacement
...
void Accelero_Sensor_Handler( void *handle,  uint32_t msTick, uint32_t *msTickStateChange, uint8_t *state)
{
  ...
  /* Stuff Jody is adding */
  float y_threshold = 70.0f;
  float static_threshold = 100.0f;
  float x_threshold = -70.0f;
  float y_d, x_d;
 ...
 */
  ...
  int32_t d1_x, d2_x, d1_y, d2_y;
  int32_t d1_vx, d2_vx, d1_vy, d2_vy, d1_ax, d2_ax, d1_ay, d2_ay, d1_dfx, d2_dfx, d1_dfy, d2_dfy;
...
//    accel_x = (float)acceleration.AXIS_X;
    ...
    if(SendOverUSB) /* Write data on the USB */
    {
    	 ...
    	 */
    		...
    		velocity_filter_x = velocity_x*iirh_0 + velocity_prev_x*iirh_1 - velocity_filter_prev_x*iirh_2;
      		velocity_filter_prev_x = velocity_filter_x;

        	/*
        	 * Integration of velocity
        	 */

    		displacement_x = displacement_x + (velocity_filter_x + velocity_filter_prev_x)*Tsample/2;
    		velocity_prev_x = velocity_x;

        	/*
        	 * High pass filter applied to displacement
        	 */

			displacement_filter_x = displacement_x*iirh_0 + displacement_prev_x*iirh_1 - displacement_filter_prev_x*iirh_2;
			displacement_prev_x = displacement_x;
			displacement_filter_prev_x = displacement_filter_x;
			...
			floatToInt(velocity_filter_x, &d1_vx, &d2_vx, 4);
			floatToInt(displacement_filter_x, &d1_dfx, &d2_dfx, 4);

	// FOR THE Y DIRECTION OF DISPLACEMENT STUFF
		/*
		 * Anti-aliasing filter applied to acceleration
		 */
		 ...
		 */
			...
			sprintf( dataOut, "%i\t%i\r\n (x, y) displacement: ",
					(int)d1_dfx, (int)d1_dfy);
    		CDC_Fill_Buffer(( uint8_t * )dataOut, strlen( dataOut ));


    		if ((*state == 0) && d1_dfy > y_threshold && (abs(d1_dfx) < static_threshold) && (msTick - *msTickStateChange) > 200) {
    			*state = 1;
    			*msTickStateChange = msTick;
    		}
    		else if ((*state == 1)) {
    			if (d1_dfx < x_threshold && (abs(d1_dfy) < static_threshold) && (msTick - *msTickStateChange) > 200) {
					*state = 2;
					*msTickStateChange = msTick;
    			}
    			else if (d1_dfx > static_threshold || (abs(d1_dfy) > static_threshold) || (msTick-*msTickStateChange) > 1000) {
    				*state = 0;
    				*msTickStateChange = msTick;
    			}
    		}
    		else if (*state == 2 && (msTick - *msTickStateChange) < 1000) {
    			sprintf(dataOut, "\n\n\r\t\t L motion detected\n");
    			CDC_Fill_Buffer((uint8_t *)dataOut, strlen(dataOut));
    			BSP_LED_On(LED1);
    			HAL_Delay(200);
